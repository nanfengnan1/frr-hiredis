...
// cli function
static int frr_hiredis_debug_config_write(struct vty *vty);
static int frr_hiredis_debug_set_all(uint32_t flags, bool set);
...
static int frr_hiredis_debug_config_write(struct vty *vty)
{
	if (DEBUG_MODE_CHECK(&nb_dbg_client_hiredis, DEBUG_MODE_CONF))
		vty_out(vty, "debug northbound client redis\n");
...
static int frr_hiredis_debug_set_all(uint32_t flags, bool set)
{
	DEBUG_FLAGS_SET(&nb_dbg_client_hiredis, flags, set);

	/* If all modes have been turned off, don't preserve options. */
	if (!DEBUG_MODE_CHECK(&nb_dbg_client_hiredis, DEBUG_MODE_ALL))
		DEBUG_CLEAR(&nb_dbg_client_hiredis);
...
static void frr_hiredis_cli_init(void)
{
	hook_register(nb_client_debug_config_write,
	      frr_hiredis_debug_config_write);
	hook_register(nb_client_debug_set_all, frr_hiredis_debug_set_all);
...
static bool frr_daemon_sub_channel(const char *daemon_name)
{
	...
	while (hiredis_sub_channels[i][0]) {
		...
		if (strncasecmp(daemon_name, hiredis_sub_channels[i][0], strlen(hiredis_sub_channels[i][0])) == 0)
			break;
		i++;
	}
